 ...
 */
public class BatteryRecycleActivity extends BaseTitleActivity {
    ...
    private void doRecycle(String name, String phone, String address) {
        ...
        map.put("BATTERY_RECORDID", batteryInfo.getBATTERY_RECORDID());
        map.put("BATTERY_THEFTNO", batteryInfo.getBATTERY_THEFTNO());
        map.put("VEHICLEBRAND", batteryInfo.getVEHICLEBRAND());
        map.put("VEHICLEMODELS",batteryInfo.getVEHICLEMODELS());
        map.put("COLORID", batteryInfo.getCOLORID());
        map.put("BATTERY_QUANTITY", batteryInfo.getBATTERY_QUANTITY()+"");
        map.put("BUYDATE", batteryInfo.getBUYDATE());
        map.put("PRICE", batteryInfo.getPRICE()+"");
        map.put("OWNERNAME", name);
        map.put("CARDID", batteryInfo.getOWNER_CARDID());
        map.put("CARDTYPE",batteryInfo.getOWNER_CARDTYPE()+"");
        map.put("PHONE", phone);
        map.put("ADDRESS", address);
        map.put("ISREGISTER", "1");//是否备案登记（0否，1是）
        ...
        HttpUtils.postK(RP, new HttpUtils.HttpCallBack() {
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (errorCode == 0) {
                        DialogUtil.showSuccess(BatteryRecycleActivity.this,"电瓶回收成功");
                    } else {
    ...
    protected void initData() {
        tvBatteryRecycleRecordId.setText(batteryInfo.getBATTERY_RECORDID());
    }


    public static void goActivity(Context context, BatteryInfo batteryInfo) {
        Intent intent = new Intent(context, BatteryRecycleActivity.class);
